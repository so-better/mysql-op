		...
		return new Promise((resolve, reject) => {
			...
			if (associatedTables instanceof Array && associatedTables.length > 0) {
				associatedTables.forEach(associatedTable => {
					sql +=
						` ${joinType} join ${associatedTable.table} on ${associatedTable.columns[1]}=${associatedTable.columns[0]}`;
				})
				sql += ` where ${column}=?`;
			} else {
				sql += ` where ${column}=?`
		...
		return new Promise((resolve, reject) => {
			...
			Object.keys(queryOptions).forEach(function(key, index) {
				...
				if (qb.value instanceof Array) {
					if (qb.value[0] != null && qb.value[1] != null && qb.value[0] != undefined && qb
						.value[1] != undefined) {
			...
			if (typeof(startIndex) == "number" && typeof(pageSize) == "number" && !isNaN(startIndex) && !
				isNaN(pageSize)) {
				sql += ` limit ?,? `;
